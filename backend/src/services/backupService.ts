...
export class BackupService {
  ...
  async createDump(dbConfig: DatabaseConfig): Promise<string> {
    ...
    const pool = this.databaseService.getPool();
    const dumpCommand = `PGPASSWORD=${pool.options.password} pg_dump -h db -p 5432 -U ${pool.options.user} -d ${dbConfig.database} -Fp -f ${dumpPath}`;
    ...
    try {
      ...
      // Insert into backup and backup_history tables
      const backupId = await this.saveBackupInfo(dumpPath, dbConfig);
      await this.saveBackupHistoryInfo(backupId, dumpPath, dbConfig.database);
  ...
  async restoreDatabase(sourceBackupId: number, targetDatabaseName: string): Promise<void> {
    try {
      ...
      const pool = this.databaseService.getPool();
      const restoreCommand = `PGPASSWORD=${pool.options.password} psql -h db -p 5432 -U ${pool.options.user} -d ${targetDatabaseName} -f "${backupPath}"`;
  ...
  private async createDatabaseIfNotExists(databaseName: string): Promise<void> {
    try {
      ...
      if (result.rows.length === 0) {
        console.log(`La base de données ${databaseName} n'existe pas. Création en cours...`);
        const pool = this.databaseService.getPool();
        const createDbCommand = `PGPASSWORD=${pool.options.password} createdb -h db -p 5432 -U ${pool.options.user} ${databaseName}`;
        await this.execCommand(createDbCommand);
        console.log(`Base de données ${databaseName} créée.`);
      } else {
        console.log(`La base de données ${databaseName} existe déjà.`);
  ...
  async getDatabaseContents(databaseName: string): Promise<any> {
    try {
      // Get all tables
      const tablesQuery = `
        SELECT table_name 
  ...
  async createDatabase(databaseName: string): Promise<void> {
    ...
    await this.databaseService.getPool().query(createDbQuery);
  ...
  async setCurrentDatabase(databaseName: string): Promise<void> {
    try {
      const newPool = await this.databaseService.createNewConnection(databaseName);
      // Test the connection
      await newPool.query('SELECT 1');
      console.log(`Successfully connected to database: ${databaseName}`);
      // You might want to store this new pool or update the existing one in your DatabaseService
      // For example:
      // this.databaseService.setPool(newPool);
    } catch (error) {
