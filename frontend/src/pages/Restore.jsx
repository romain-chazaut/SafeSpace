...
import { FaDatabase, FaArrowAltCircleUp, FaSpinner, FaExclamationCircle, FaLock, FaCheckCircle } from 'react-icons/fa';
...
const RestoreComponent = () => {
  ...
  if (!isLoggedIn) {
    return (
      <div className="restore-container">
        <h1 className="restore-title"><FaLock /> Accès Restreint</h1>
        <p className="restore-description">Veuillez vous connecter pour accéder à la fonctionnalité de restauration de base de données.</p>
      </div>
    );
  }
  ...
  return (
    <div className="restore-container">
      <h1 className="restore-title"><FaArrowAltCircleUp /> Restauration de Base de Données</h1>
      ...
      <div className="restore-form">
        ...
        <form onSubmit={handleRestore}>
          ...
          <button type="submit" className="restore-button" disabled={isLoading}>
      ...
      {message.content && (
        <div className={`restore-${message.type}`}>
          {message.type === 'error' ? <FaExclamationCircle /> : <FaCheckCircle />} {message.content}
        </div>
      )}
    </div>
  );
